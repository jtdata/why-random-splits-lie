...
# %% [markdown]
# ## Compare: what a random split looks like on the same timeline
#
# The diagram above is what one correctly-designed scoring occasion looks
# like: a clean boundary at as-of, and a label window that opens only after
# it. Most tutorials never build that boundary — they shuffle a table of
# rows and put 80% in training, 20% in testing, with no regard for where in
# time each row falls. The chart below scatters synthetic customer
# observations across the same span shown above and splits them 80/20 at
# random, to make that concrete before `01_naive_baseline` reports exactly
# this kind of split.
